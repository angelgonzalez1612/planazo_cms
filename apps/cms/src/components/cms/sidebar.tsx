...
export function Sidebar({
  ...
  return (
    <aside
      className={`flex h-full flex-none flex-col border-r border-border bg-card transition-[width] duration-200 ${
    ...
    >
        ...
        {NAV_GROUPS.map((group) => (
          <div key={group.label} className="flex flex-col gap-px">
            ...
            {group.items.map((item) => {
              ...
              return href ? (
                <Link key={item.id} href={href} title={collapsed ? item.name : undefined} className={className}>
                  {content}
                </Link>
              ) : (
                <button key={item.id} type="button" title={collapsed ? item.name : "Próximamente"} className={className} disabled>
                  {content}
                </button>
              );
            })}
      ...
      <div className={`flex-none border-t border-border-soft ${collapsed ? "p-2" : "p-2.5"}`}>
        ...
        >
          ...
          {!collapsed && <span className="flex-1 text-left">Colapsar menú</span>}
        </button>
      </div>
    </aside>
  );
